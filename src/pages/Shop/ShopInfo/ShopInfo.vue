...
export default{
    ...
    mounted(){
        //1. 如果没有info数据，直接结束，即不是从shop的其他路由跳转到shopInfo路由
        //直接进来的路由就是http://localhost:8080/#/shop/info,就会有
        //this.info.pics.length的报错
        if(!this.info.pics){
          return;
        }
        //2. 有info的数据，可以创建BScroll对象形成滑动
        //即为shopGoods路由到shopInfo路由
        this._initScroll();
        //3. 但是此时应包含即为shopGoods路由到shopInfo路由之后，在刷新的情况
        //也会报错的
    ...
    methods:{
        _initScroll(){
            ...
            const liWidth = 120;
            const spacing = 6;
            const len = this.info.pics.length;
            ul.style.width = (liWidth + spacing ) * len - spacing + 'px';
